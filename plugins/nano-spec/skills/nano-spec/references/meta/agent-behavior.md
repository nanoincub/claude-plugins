...
| **Iron Law (verification)** | Antes de qualquer claim "pronto/passou/feito" e antes do commit | Ver [meta/verification.md](verification.md). Evidência fresh **nesta mensagem**, sem exceção. |
...
- Security: ver [security.md](../review/security.md) — recomendado usar skill de segurança específica da stack do projeto
...
Quando receber feedback de code review (externo ou de subagent), o agente DEVE seguir o protocolo em [receiving-feedback.md](../review/receiving-feedback.md): READ → UNDERSTAND → VERIFY → EVALUATE → RESPOND → IMPLEMENT. NUNCA implementar feedback cegamente — verificar tecnicamente primeiro. Push back com raciocínio técnico se errado. Zero performative agreement ("você está totalmente certo!", "great point!", "thanks for...").
...
| TDD | [execute/tdd/tdd.md](../execute/tdd/tdd.md) — Iron Law + RED→Verify RED→GREEN→Verify GREEN→REFACTOR |
| Debug | [execute/systematic-debugging/debug.md](../execute/systematic-debugging/debug.md) — Iron Law "no fix without root cause" + 4 fases + 4.5 (questionar arquitetura) |
| Subagent por task | [execute/subagents/subagents.md](../execute/subagents/subagents.md) — implementer → spec compliance review → code quality review (ordem obrigatória) |
| Dispatch paralelo | [execute/subagents/parallel-dispatch.md](../execute/subagents/parallel-dispatch.md) — múltiplos subagents em uma mensagem + check de conflitos pós-retorno |
| Verificação formal | [verification.md](verification.md) — Iron Law: evidência fresh antes de qualquer claim |
| Code review | [review/code-review.md](../review/code-review.md) — template em [code-reviewer-prompt.md](../review/code-reviewer-prompt.md); Protocolo Dois-Eixos para pre-commit Large/Complex |
| Recepção de feedback | [review/receiving-feedback.md](../review/receiving-feedback.md) — READ → UNDERSTAND → VERIFY → EVALUATE → RESPOND → IMPLEMENT |
